...
MODEL = Path("security-knowledge/standards/gost-r-59548-2022-event-registration-core-part6-v1.yaml")
FIXTURES = Path("security-knowledge/standards/gost-r-59548-2022-event-registration-core-part6-regression-v1.json")
...
def main():
    ...
    def evaluate(case):
        ...
        if query == "schema_property": return schemas[case["schema"]][case["property"]]
        if query == "official_status_boundary": return model["verification_boundary"]["official_status_and_scope"]
        if query == "modeled_clause_boundary": return model["verification_boundary"]["clauses_6_2_57_to_6_2_68"]
        if query == "remaining_clause_boundary": return model["verification_boundary"]["clauses_6_2_69_to_6_2_81"]
    ...
    assert model["status"] == "VERIFIED_BOUNDED_PARTIAL_CLAUSE_MODEL_6_2_57_TO_6_2_68"
    assert list(schemas) == [f"G59548-6.2.{i}" for i in range(57, 69)]
    failures = []
    ...
    if failures:
        ...
        raise SystemExit(1)
    print("PASS: 12 schemas; 101 required and 34 additional placements; cumulative 68/81; 64 fail-closed cases")
