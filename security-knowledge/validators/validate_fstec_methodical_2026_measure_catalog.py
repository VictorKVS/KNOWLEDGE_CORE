...
def evaluate(case, model, families, measures):
    ...
    if query == "content_completeness":
        return "IAF_COMPLETE_UPD_RSB_ZSV_ZKO_ZEP_ZVT_ZPI_ZKU_AND_ZMU_OTHER_32_PENDING" if model["verification_boundary"]["all_measure_implementation_requirements"] == "PARTIAL_IAF_COMPLETE_UPD_RSB_ZSV_ZKO_ZEP_ZVT_ZPI_ZKU_AND_ZMU_64_OF_96" else None
...
def main():
    ...
    assert model["status"] == "VERIFIED_BOUNDED_CATALOG_SELECTION_IAF_UPD_RSB_ZSV_ZKO_ZEP_ZVT_ZPI_ZKU_ZMU_PUBLIC_TEXT_CROSSCHECK"
    ...
    assert model["verification_boundary"]["exact_class_matrix_cells"] == "PARTIAL_IAF_COMPLETE_UPD_RSB_ZSV_ZKO_ZEP_ZVT_ZPI_ZKU_AND_ZMU_384_CELLS_VERIFIED_REMAINDER_PENDING"
    failures = []
    ...
    if failures:
        for failure in failures:
            print("FAIL", failure)
        raise SystemExit(1)
    print("PASS: 17 families; 96 unique measure codes; IAF, complete UPD, RSB, ZSV, ZKO, ZEP, ZVT, ZPI, ZKU and ZMU detail linked; 36 fail-closed cases")
