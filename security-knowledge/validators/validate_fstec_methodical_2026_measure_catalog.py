...
def evaluate(case, model, families, measures):
    ...
    if query == "content_completeness":
        return "IAF_AND_UPD_1_2_COMPLETE_OTHER_90_PENDING" if model["verification_boundary"]["all_measure_implementation_requirements"] == "PARTIAL_IAF_AND_UPD_1_2_COMPLETE_6_OF_96" else None
...
def main():
    ...
    assert model["status"] == "VERIFIED_BOUNDED_CATALOG_SELECTION_IAF_AND_UPD_1_2_PUBLIC_TEXT_CROSSCHECK"
    ...
    assert model["verification_boundary"]["exact_class_matrix_cells"] == "PARTIAL_IAF_AND_UPD_1_2_36_CELLS_VERIFIED_REMAINDER_PENDING"
    failures = []
    ...
    if failures:
        for failure in failures:
            print("FAIL", failure)
        raise SystemExit(1)
    print("PASS: 17 families; 96 unique measure codes; IAF and UPD.1-UPD.2 detail linked; 36 fail-closed cases")
