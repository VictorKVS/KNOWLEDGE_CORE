...
def evaluate(case, model, families, measures):
    ...
    if query == "content_completeness":
        return "ALL_96_MEASURE_CONTENTS_COMPLETE_AT_PUBLIC_TEXT_CROSSCHECK_BOUNDARY" if model["verification_boundary"]["all_measure_implementation_requirements"] == "VERIFIED_PUBLIC_TEXT_CROSSCHECK_96_OF_96" else None
...
def main():
    ...
    assert model["status"] == "VERIFIED_BOUNDED_COMPLETE_96_MEASURE_CONTENT_PUBLIC_TEXT_CROSSCHECK"
    ...
    assert model["verification_boundary"]["exact_class_matrix_cells"] == "VERIFIED_PUBLIC_TEXT_CROSSCHECK_576_OF_576"
    failures = []
    ...
    if failures:
        for failure in failures:
            print("FAIL", failure)
        raise SystemExit(1)
    print("PASS: 17 families; 96 unique measure codes; all 17 family details linked; 96/96 content complete and 576/576 class cells verified at public-text crosscheck boundary; 36 fail-closed cases")
