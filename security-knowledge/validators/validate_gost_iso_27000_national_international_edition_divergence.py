...
MODEL=ROOT/'security-knowledge/standards/gost-iso-27000-national-international-edition-divergence-v1.yaml'
FIXTURES=ROOT/'security-knowledge/standards/gost-iso-27000-national-international-edition-divergence-regression-v1.json'
OBS=ROOT/'security-knowledge/evidence/gost-iso-27000-national-international-edition-divergence-observation-2026-09-02.yaml'
...
assert sum(x['divergence'].startswith('NATIONAL_SOURCE_WITHDRAWN') for x in model['standards'])==5
assert sum(x['divergence'].startswith('ALIGNED') for x in model['standards'])==1
...
for path in [str(MODEL.relative_to(ROOT)),str(FIXTURES.relative_to(ROOT)),str(OBS.relative_to(ROOT))]: assert path in master
print('PASS: 6 selected GOST/ISO edition routes; 5 divergences; 64/64 rules/cases; red-team 12/12; evidence 18/18')
