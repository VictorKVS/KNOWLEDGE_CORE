...
MODEL = Path("security-knowledge/standards/gost-r-59548-2022-event-registration-core-part2-v1.yaml")
FIXTURES = Path("security-knowledge/standards/gost-r-59548-2022-event-registration-core-part2-regression-v1.json")
...
def main():
    ...
    def evaluate(case):
        ...
        if query == "malware_duplicate_count": return model["literal_value_sets"]["MALWARE_OBJECT_TYPE"]["values"].count("MALICIOUS_PROGRAM")
        ...
        if query == "malware_duplicate_marker": return model["literal_value_sets"]["MALWARE_OBJECT_TYPE"]["literal_duplicate_preserved"]
        ...
        if query == "modeled_clause_boundary": return model["verification_boundary"]["clauses_6_2_9_to_6_2_20"]
        if query == "remaining_clause_boundary": return model["verification_boundary"]["clauses_6_2_21_to_6_2_81"]
    ...
    assert model["status"] == "VERIFIED_BOUNDED_PARTIAL_CLAUSE_MODEL_6_2_9_TO_6_2_20"
    assert list(schemas) == [f"G59548-6.2.{i}" for i in range(9, 21)]
    failures = []
    ...
    if failures:
        ...
        raise SystemExit(1)
    print("PASS: 12 schemas; 76 required and 72 additional placements; cumulative 20/81; 64 fail-closed cases")
