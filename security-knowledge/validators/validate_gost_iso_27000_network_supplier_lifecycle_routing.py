...
MODEL = ROOT / 'security-knowledge/standards/gost-iso-27000-network-supplier-lifecycle-routing-v1.yaml'
FIXTURES = ROOT / 'security-knowledge/standards/gost-iso-27000-network-supplier-lifecycle-routing-regression-v1.json'
OBS = ROOT / 'security-knowledge/evidence/gost-iso-27000-network-supplier-lifecycle-routing-observation-2026-09-02.yaml'
...
assert sum(x['divergence'].startswith('ALIGNED_') for x in model['standards']) == 3
assert sum(not x['divergence'].startswith('ALIGNED_') for x in model['standards']) == 3
assert sum(x['national_conformity'] == 'NEQ' for x in model['standards']) == 1
...
print('PASS: 6 selected routes; 3 aligned; 3 edition divergences; 1 NEQ; 64/64 rules/cases; red-team 12/12; evidence 18/18')
