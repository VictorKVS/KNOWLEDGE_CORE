...
MODEL = ROOT / 'security-knowledge/standards/gost-iso-27000-application-supplier-digital-evidence-edition-routing-v1.yaml'
FIXTURES = ROOT / 'security-knowledge/standards/gost-iso-27000-application-supplier-digital-evidence-edition-routing-regression-v1.json'
OBS = ROOT / 'security-knowledge/evidence/gost-iso-27000-application-supplier-digital-evidence-edition-routing-observation-2026-09-02.yaml'
...
assert sum(x['divergence'].startswith('ALIGNED_') or x['divergence'].startswith('BASE_EDITION_ALIGNED_') for x in model['standards']) == 5
assert sum('WITHDRAWN_INTERNATIONALLY_REPLACED' in x['divergence'] for x in model['standards']) == 1
assert sum('CORRIGENDUM_ADOPTION_PENDING' in x['divergence'] for x in model['standards']) == 1
...
print('PASS: 6 selected routes; 5 aligned base editions; 1 edition divergence; 1 corrigendum pending; 64/64 rules/cases; red-team 12/12; evidence 18/18')
